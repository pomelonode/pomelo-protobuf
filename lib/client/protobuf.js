...
(function (exports, global){
  ...
  function checkMsg(msg, protos){
    ...
    for(var name in protos){
      ...
      switch(proto.option){
        ...
        case 'optional' :
          if(typeof(msg[name]) !== 'undefined'){
            var message = protos.__messages[proto.type] || MsgEncoder.protos[proto.type];
            if(!!message){
  ...
  function encodeProp(value, type, offset, buffer, protos){
    switch(type){
      ...
      break;
      default :
        var message = protos.__messages[type] || MsgEncoder.protos[type];
        if(!!message){
  ...
  function decodeProp(type, protos){
    switch(type){
      ...
      case 'string' :
        ...
        return str;
      default :
        var message = protos && (protos.__messages[type] || MsgDecoder.protos[type]);
        if(!!message){
          var length = codec.decodeUInt32(getBytes());
          var msg = {};
