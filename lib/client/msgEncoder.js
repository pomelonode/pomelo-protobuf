...
MsgEncoder.encode = function(route, msg){	
	...
	var length = encoder.byteLength(JSON.stringify(msg));
...
function encodeProp(value, type, offset, buffer, protos){
	switch(type){
		...
		break;
		case 'float':
			writeBytes(buffer, offset, encoder.encodeFloat(value));
			offset += 4;
		break;
		case 'double':
			writeBytes(buffer, offset, encoder.encodeDouble(value));
			offset += 8;
		break;
		case 'string':
			var length = encoder.byteLength(value);
			
			//Encode length
			offset = writeBytes(buffer, offset, encoder.encodeUInt32(length));
			//write string
			encoder.encodeStr(buffer, offset, value);
			offset += length;
		...
		default :
			if(!!protos.__messages[type]){
				...
				var tmpBuffer = new ArrayBuffer(encoder.byteLength(JSON.stringify(value)));
				var length = 0;
